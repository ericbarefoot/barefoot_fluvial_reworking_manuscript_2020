...
\title{Grain-size sorting in floodplains due to climate-driven \\ precipitation seasonality during the PETM }
\author{Eric Barefoot, Jeff Nittrouer, Brady Foreman, Gerald Dickens, Elizabeth Hajek}
\date{\today}
...
Accumulation of terrigenous mud in marginal shelf environments and coarsening of terrestrial deposits during the PETM are both used to infer an increase in water and sediment discharge in fluvial systems.
...
Herein, we use measurements of channel geometry, sediment grain size, channel bar structures, and avulsion deposits to assess the fluvial response to hydrological change during the PETM.
...
In marginal marine environments, the PETM is most commonly associated with increased deposition of terrigenous clay \parencite{john_north_2008, gibson_stratigraphic_2000}, which is attributed to elevated sediment flux from continents during the PETM: a so-called ``system-clearing'' event affecting rivers and floodplains on a global scale \parencite{foreman_fluvial_2012, jerolmack_shredding_2010}.

Studies of terrestrial alluvial systems generally find that strata deposited during the PETM are channel-dominated and coarser than bounding units \parencite[e.g.][]{pujalte_massive_2015}.
...
However, the paleohydraulic techniques used to reconstruct discharge in the geologic record carry significant uncertainty \parencite[$1.3\times$ to $14\times$ increase in][]{chen_estimating_2018}, and it remains unclear if sedimentological differences in PETM strata represent an increase in the average discharge and sediment flux.
In an alternative scenario, the mean discharge averaged over annual to decadal time scales may have remained steady, while runoff on short time scales was more variable, producing wetter wet seasons and drier dry seasons \parencite{schmitz_abrupt_2007}.
...
Juxtaposed between mud-rich units, sedimentological characteristics of the sandy Molina member contrast starkly with the underlying Atwell Gulch and the overlying Shire members of the Wasatch \parencite{donnell_paleocene_1969}.
...
Shields parameter estimates using the combined depth-slope product shows that within uncertainty, sediment transport was also consistent between the Molina and the bounding members (see Table \ref{tab:paleo_geom}).
...
\mnote[Our data also indicate that avulsion style changed during Molina time, progradational/transitional avulsions were more abundant than incisional-type avulsions.]{this is what we would like Liz to contribute}
...
Given that tectonic subsidence in the Piceance Basin was stable throughout the PETM \parencite{foreman_fluvial_2012}, enhanced fluvial reworking of floodplain deposits implies that channel migration rates accelerated as a result of changing hydroclimate.
...
On the other hand, evidence for more active bar reworking in the Molina member supports the assessment that short-term runoff variability did increase during the PETM.
...
Avulsion deposits are also dominated by sandy crevasse splays in the Molina member, indicating that elevated bed shear stress during high intensity flows may have suspended bed material and transporting coarse sediment overbank \parencite{rouse_experiments_1939}.
