...
%   Preservation is calculated for all channel elements, and requires the Polygon Clip package on the Matlab
%   exchange. Model output is saved as a .mat file in the last cell
%
...
ch_w(1)=11; %initial sand body width (generic length units) MUST BE AN ODD NUMBER=
    ...
    deswidth = 3; % desired window width (scalar multiple of ch_w)
    desthick = 12; % desired window thickness (scalar multiple of ch_z)
...
% AVULSION PATTERN 
avuls_type = 0*ones(1,nits); % vector with flags for clustered (0), compensation (1), or random avulsions (2)
...
fpagg_area = nan(nits,1);       % vectore to store floodplain area deposited each timestep
...
for t = 2:nits
    ...
    if isnan(lat_dist)==0
        lat_mob(t) = randsample(lat_dist,1);
    end
...
%%%%%%%%%%%%%%%%% COMPENSTION TIMESTEPS
    if avuls_type(t-1) == 1
        ...
        smoothtopo = moving(Topo(t-1,:),floor(ch_w(t)));
...
%%%%%%%%%%%%%%%%%%%% DETERMINE FLOODPLAIN AND CHANNEL DEPOSITION LOCATIONS
    ...
    %%%% determine channel indexes
            ...
            S_left = ch_idx(t)-edge_buff:ch_idx(t)-1;
            S_right = ch_idx(t):ch_idx(t)+edge_buff;
            S_range = [S_left S_right];

    %%%% determine floodplain indexes            
         %%%%% OPTION 1: FLOODPLAIN EVERYWHERE BESIDES CHANNEL
        fp_range = setdiff(model_idx,S_range);  %index values of floodplain locations
        
         %%%%% OPTION 2: FLOODPLAIN ONLY WITHIN X CHANNEL WIDTHS ON EITHER SIDE
...
%%%%%% FROM MSB MODEL
    ...
    % update floodplain elevation
          ...
          for m=1:length(toohigh_idx)
            ...
            else Topo(t,toohigh_idx(m)) = Topo(t-1,toohigh_idx(m));
            end
          end
...
%     %%% OPTION 2: Color code rectangles by timestep
            cc=hsv(nits);   
            rectangle('Position',chan_pos(t,:),'FaceColor',cc(t,:)); % this plots the rectangular channel object on a figure
...
% 
    %%Set axis properties
           %axis equal
           set(gca,'xlim',[0,model_w],'ylim',[0,100]); %[0,(max(max(Topo))+1)])  %,'ylim',[0,nits/(model_w/4)])
...
for tt = 2:(size(p_pres,1)-1)   % time counter
        ...
        for ii = 1:(size(p_pres,2)-tt)  % channel object (rect) counter
            ...
            else overlap_area = rectint(chan_pos(tt,:),chan_pos(tt+ii,:));
                if overlap_area > 0
        ...
        ch_touch(tt)=length(et);
            ...
            for e = 1:length(eros_time)
                ...
                % Find the vertical overlap of eroding channel with original
                    ...
                    else ch_int(e,2) = chan_pos(eros_time(e),2); %if overlying channel partially erodes
...
sigpres1 = length(find(vert_pres(ind1) <= 0.99*ch_z & vert_pres(ind1) > 0.75*ch_z & (final_pres(ind1)>=50)))/length(ind1)*100; % max vert erosion >75% & preserved area > 50%
sigpres2 = length(find(vert_pres(ind1) >= 0.75*ch_z & final_pres(ind1)<50)) / length(ind1)*100; % max vert erosion > 75% ch_z, but preserved area < 50%
...
% % F=getframe;
% % bw = im2bw(F.cdata);
% % n2g = length(find(bw==0))/(size(bw,1)*size(bw,2))*100;
